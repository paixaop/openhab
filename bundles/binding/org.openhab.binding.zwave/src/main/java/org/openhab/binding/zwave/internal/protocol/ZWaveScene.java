...
/**
 * This class provides a storage class for Z-Wave scenes within the node class.
...
public class ZWaveScene {
	...
	// 0 - take the value on the device as the node level, 1 set the device to
	 ...
	 * 
	 * @param duration
	 *            0x00 - Instant 0x01-0x7F - duration in seconds 0x80-0xFE -
	 *            duration in minutes 0xFF - Factory default
	 */
	 ...
	 * 
	 * @return dimming duration 0x00 - Instant 0x01-0x7F - duration in seconds
	 *         0x80-0xFE - duration in minutes 0xFF - Factory default
	 */
	...
	/**
	 * Set the override value for the scene. When override is "OFF" or 0 a scene
	 * is programmed using the current device's value as the scene stored value.
	 * If override is "ON", or 1, the current device value is ignored and the
	 * value provided by the controller is used to configure the scene.
	 * 
	 * @param newOverride
	 */
	...
	public ArrayList<Integer> getNodesSupportingSceneActivation() {
		...
		for (ZWaveSceneDevice device : devices.values()) {
			...
			if (node == null) {
				logger.error("Scene {} has a device without node information",
						sceneId);
				return null;
			}
			ZWaveSceneActivationCommandClass sceneActivationCC = (ZWaveSceneActivationCommandClass) node
					.getCommandClass(CommandClass.SCENE_ACTIVATION);
	...
	public void programSceneControllersWithNonSceneCapableDevices() {
		...
		for (ZWaveSceneController sceneController : sceneControllers.values()) {
			...
			ZWaveConfigurationCommandClass configurationCmdClass = (ZWaveConfigurationCommandClass) node
					.getCommandClass(CommandClass.CONFIGURATION);

			// First program all scene devices that do not support SCENE
			// ACTIVATION Command Class
			if (!basicDevices.isEmpty()) {
				for (Integer value : basicDevices.keySet()) {
					logger.info(
							"NODE {} Program scene {} associations for group {}. Adding non scene capable nodes: {}",
							node.getNodeId(), sceneId, groupId, basicDevices
									.get(value).toString());
					...
					// Set associations
					SerialMessage msg = associationCmdClass
							.setAssociationMessage(groupId, nodes);
					...
					controller.sendData(msg);
					logger.info(
							"NODE {} Configuration for group {} with value {} sent",
							node.getNodeId(), groupId, value);
				}
			}
		}
	}
	...
	public void programSceneControllersWithSceneCapableDevices() {
		...
		for (ZWaveSceneController sceneController : sceneControllers.values()) {
			...
			ZWaveSceneControllerConfCommandClass sceneControllerCmdClass = (ZWaveSceneControllerConfCommandClass) node
					.getCommandClass(CommandClass.SCENE_CONTROLLER_CONF);
			logger.info(
					"NODE {} Program controller with scene {} group {} dimming duration: {}",
					node.getNodeId(), sceneId, groupId, dimDurationToString());
			SerialMessage msg = sceneControllerCmdClass.setValueMessage(
					(byte) groupId, (byte) sceneId, dimmingDuration);
			controller.sendData(msg);
			...
			if (sceneNodes == null) {
				logger.info(
						"NODE {} Program scene {} has no nodes! Stopping scene programming.",
						node.getNodeId(), sceneId);
				return;
			}

			logger.info(
					"NODE {} Program scene {} associations for group {}. Adding scene capable nodes: {}",
					node.getNodeId(), sceneId, groupId, sceneNodes.toString());

			// Add the Z-Wave controller to the association group so it can
	...
	public void programSceneCapableNodes() {
		...
		for (Integer nodeId : nodes) {
			...
			if (device == null) {
				logger.error("NODE {} is not associated with a Scene Device.",
						nodeId);
				return;
			}
			...
			logger.info(
					"NODE {} Program scene {} level {} dimming duration: {}",
					node.getNodeId(), sceneId, device.getValue(),
					dimDurationToString());

			SerialMessage msg = sceneActuatorCmdClass.setValueMessage(
					(byte) sceneId, device.getValue(), dimmingDuration,
					override == 1);
			controller.sendData(msg);
		}
	}
	 ...
	 * 
	 * @param nodeId
	 *            id of the node being added to the scene
	 * @param d
	 *            ZWaveSceneDevice with the values and node information to set
	 *            when scene is activated
	 */
	 ...
	 *            id of the node being added to the scene
	 * @param value
	 *            of the node that should be set when scene is activated
	 */
	public void addDevice(int nodeId, int value) {

		ZWaveNode node = controller.getNode(nodeId);
		if (node == null) {
			logger.error("Scene {} Cannot add device with null node object",
					sceneId);
			return;
		}
